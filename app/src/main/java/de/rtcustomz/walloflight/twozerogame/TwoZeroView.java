...
public class TwoZeroView extends View {

    //Internal Constants
    static final int BASE_ANIMATION_TIME = 100000000;
    private static final String TAG = TwoZeroView.class.getSimpleName();
...
//    private static final String WOL_SIMULATOR_IP = "192.168.188.44";
    ...
    public TwoZeroView(Context context) {
        super(context);

        //Loading resources
        game = new MainGame(context, this);
        sendBitmapTask = new SendBitmapTask(context, false);
//        Client.setGamma(2.8);
//        Client.setIP(WOL_SIMULATOR_IP);
        try {
            //Getting assets
...
//    }
    ...
    public void sendBitmap() {
        switch(sendBitmapTask.getStatus()) {
            ...
            case PENDING:
                Bitmap grid = extractGrid();
                sendBitmapTask.execute(grid);
                break;
        }
    }

    private Bitmap extractGrid() {
        int width = gridWidth + (cellSize + gridWidth) * game.numSquaresX;
        int height = gridWidth + (cellSize + gridWidth) * game.numSquaresY;

        setDrawingCacheEnabled(true);
        Bitmap grid = Bitmap.createBitmap(getDrawingCache(), startingX, startingY, width, height);
        destroyDrawingCache();
